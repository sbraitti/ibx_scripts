...
param(
    ...
    [switch]$NoCopy = $false,
    [string]$RemoteShareBase = "//path/to/save",
    [int]$UseNetRetry = 2,
    [int]$NetRetryDelaySeconds = 2
)

# --- Configuration (edit as needed) ---
# When running with -NoCopy, outputs are written to the current working directory and no remote copy is attempted.
if ($NoCopy) {
    try { $OutDir = (Get-Location).Path } catch { $OutDir = "." }
} else {
...
# Determine target server and IPs
# If $RemoteServer is provided, target that server; otherwise target the local machine.
$LocalServer = if ($RemoteServer) { $RemoteServer } else { $env:COMPUTERNAME }

# Get a list of local IPv4 addresses for the source machine (used as fallback)
$LocalIPs = Get-LocalIPv4Addresses
if (-not $LocalIPs -or $LocalIPs.Count -eq 0) { $LocalIPs = @('127.0.0.1') }

# If targeting a remote server, attempt DNS resolution to provide a ServerIPField using that remote host's A records.
if ($RemoteServer) {
    try {
        $resolved = [System.Net.Dns]::GetHostAddresses($LocalServer) | Where-Object { $_.AddressFamily -eq 'InterNetwork' } | ForEach-Object { $_.ToString() }
        if ($resolved -and $resolved.Count -gt 0) {
            $ServerIPField = ($resolved -join ';')
        } else {
...
if ($RemoteServer) {
    Log "Starting dnsgetinfo_v2 targeting remote server: $LocalServer (Resolved IPs: $ServerIPField)"
} else {
    Log "Starting dnsgetinfo_v2 on local server: $LocalServer (IPs: $ServerIPField)"
}
...
function Process-LocalServer {
    ...
    foreach ($z in $zones) {
        ...
        $dynamicRecordCountBefore2024 = $null
        ...
        try {
            ...
            $dynamicRecordCountBefore2024 = 0
            Get-DnsServerResourceRecord -ComputerName $srv -ZoneName $zoneName -ErrorAction Stop | ForEach-Object {
                $recordCount++
                try {
                    if ($_.PSObject.Properties.Match('Timestamp') -and $_.Timestamp) {
                        $dynamicRecordCount++
                        try {
                            $ts = [datetime]$_.Timestamp
                            if ($ts -lt [datetime]'2024-01-01') { $dynamicRecordCountBefore2024++ }
                        } catch {}
                    }
                } catch {}
            }
        ...
        $rows.Add([pscustomobject]@{
            ...
            DynamicRecordCountBefore2024 = $dynamicRecordCountBefore2024
            SOA_MName            = $soaMName
...
try {
    ...
    if ($csvPath) {
        if ($NoCopy) {
            Log "NoCopy specified; CSV left in: $OutDir"
        } else {
